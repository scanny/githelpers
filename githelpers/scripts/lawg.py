...
class LogLines(list):
    """
    The collection of all the |Line| objects in the git log generated by the
    ...
    def __str__(self):
        ...
        return '\n'.join(
            [line.pretty(max_graf, max_sha1, max_time) for line in self]
        )

    @classmethod
    def load(cls):
        """
        Return a |LogLines| object filled with the results of the git log
        requested. Command-line parameters are passed through to the git log
        command.
        """
...
class Line(object):
    ...
    def pretty(self, max_graf, max_sha1, max_time):
        ...
        return '%s%s%s  %s%s  %s%s' % (
            graf, self.sha1, sha1_pad, self.time, time_pad, self.subj,
            self.refs
        )
    ...
    def _condition(cls, line):
        """
        Remove extraneous information from line, like ' ago' and the
